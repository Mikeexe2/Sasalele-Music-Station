...
function setRandomImage() {
    ...
    randomimg.classList.remove('anim-slide-left', 'anim-slide-right', 'anim-slide-top', 'anim-slide-bottom');
    void randomimg.offsetWidth;

    const animations = ['anim-slide-left', 'anim-slide-right', 'anim-slide-top', 'anim-slide-bottom'];
    const animClass = animations[getRandomInt(0, animations.length - 1)];
    randomimg.classList.add(animClass);

    const randRotate = getRandomInt(-2, 2);
    const randScale = 1 + (Math.random() * 0.04 - 0.02);
    randomimg.style.transform = `rotate(${randRotate}deg) scale(${randScale})`;
}
...
function updateImageAndBackground() {
    if (currentPhase === 'random') {
        ...
        if (gifFolderCurrentIndex > gifFolderImageCount) {
            ...
            animationTimeout = setTimeout(updateImageAndBackground, 50);
        }
    }
}
