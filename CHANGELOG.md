...
- Instant search icon to quickly search for the currently playing track title.
- Detailed individual tags on radio stations and more categories of them.
- Progressive Web App (PWA) support.
- The footer now displays the last updated date of the database.
- Random wallpaper feature.
- Integrated MPEG-DASH support into the player.
- Added functionality to extract metadata from google drive files.
- Display metadata (title, artist, lyric) in the GDMP.
- Added a limit to the number of messages loaded initially in the chat interface.
